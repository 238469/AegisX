...
4. 输出格式：
   必须输出 JSON: {{"test_cases": [ {{"parameter": "参数名", "payload": "Payload值"}} ]}}
   
   - 输出: `[ {{"parameter": "id", "payload": "123&admin=1"}} ]` (利用拼接注入 admin)
   
   **注意**：
    1. **首要目标**是发现隐藏参数（通过参数拼接）和枚举敏感值。
    2. 请勿生成 SQL注入、XSS 或 路径遍历 等 Payload，那是其他模块的职责。
    3. 所有的 Payload 必须通过 HTTP 参数传递，不要修改 URL 路径结构。"""
